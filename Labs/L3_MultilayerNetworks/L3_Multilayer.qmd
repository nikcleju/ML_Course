...
Use a **softmax** activation in the output layer so that the outputs form a proper probability distribution (they sum to 1).
...
![Illustration of a MLP network with 3 hidden layers](img/MLP.png){width=400}



### Matrix form

**One neuron** does a linear combination of the inputs, followed by an activation function:
$$\begin{bmatrix} w_1 & w_2 & \dots & w_N & b \end{bmatrix} \cdot \begin{bmatrix} x_1 \\ x_2 \\ \vdots \\ x_N \\ 1 \end{bmatrix} = z
\rightarrow \sigma(z) = a$$

$$\begin{bmatrix} w_1 & w_2 & \dots & w_N \end{bmatrix} \cdot \begin{bmatrix} x_1 \\ x_2 \\ \vdots \\ x_N \end{bmatrix}  + b = z
\rightarrow \sigma(z) = a$$
...
The activation function can be the **sigmoid**, **softmax**, **ReLU**, **tanh** etc. Typically the outputs use a sigmoid activation for binary outputs, and a **softmax** for multi-class outputs (so that the class scores sum to 1), whereas the hidden layers can use whichever activation function the designer prefers.

- sigmoid activation function:
  $$f(z) = \sigma(z) = \frac{1}{1 + e^{-z}} = \frac{e^z}{e^z + 1}$$

- softmax activation function (for multi-class outputs $z_1, z_2, \dots z_N$):
  $$f(z_i) = \frac{e^{z_i}}{\sum_{j=1}^N e^{z_j}}, \textrm{ for each } z_i$$
...
For a single input, whose output is a vector $\hat{y} = [\hat{y}_1, \dots, \hat{y}_N]$ (assuming $N$ classes)
and true output should have been $y = [y_1, \dots, y_N]$, the cross-entropy function is defined as:

$$L(y, \hat{y}) = - \sum_{i=1}^{N} y_i \log{\hat{y}_i} = -\log{\hat{y}_{\text{true class}}},$$

where $\hat{y}_{\text{true class}}$ is the model's predicted probability for the true class of the input. We assume one-hot encoding, i.e. the vector $y$
...
and
$$\frac{dJ}{da^{1}} = \frac{dJ}{da^2} \cdot \frac{da^{2}}{da^{1}}$$

Finally, the input layer computes its own derivatives, $\frac{da^{1}}{dW^{1}}$, and together with the $\frac{dJ}{da^{1}}$ received from the layer above, computes:
...
2. Knows how to compute its own derivatives $\frac{dO}{dP}$ and $\frac{dO}{dI}$.
...
5. Computes $\frac{dJ}{dI} = \frac{dJ}{dO} \cdot \frac{dO}{dI}$ and passes them back to the preceding layer.
...
b. Compute the network output for the same input, considering that the hidden layer has ReLU activation instead (output layer still sigmoid)

c. Compute the cost function (i.e. the loss), assuming the true output should have been $y = 0$.

d. Compute the cost function (i.e. the loss), assuming the true output should have been $y = 1$.

e. Suppose we have a network with input size 100, one hidden layer with 1000 neurons, and one output layer with 4 neurons.
...
1. Prepare the data
   ...
   ```matlab
   % Reindex so that quality 3 -> class 1, quality 4 -> class 2, etc.
   Yenc = full(ind2vec((Y' - 2)));
...
2. Load the data (this will create the matrix `mnist_train`):
   ...
   The first column is the digit number. The remaining 784 columns contain the pixels in the linearized image (every image has 28x28 pixels).
